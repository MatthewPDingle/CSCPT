...
class ConnectionManager:
    ...
    async def connect(
        self, websocket: WebSocket, game_id: str, player_id: Optional[str] = None
    ):
        """
        ...
        logging.warning(
            f"Beginning connect process for WebSocket {id(websocket)} to game {game_id}"
        )
        ...
        async with self.lock:
            ...
            if player_id:
                for existing_ws in list(self.active_connections.get(game_id, set())):
                    if self.socket_player_map.get(existing_ws) == player_id:
                        logging.warning(
                            f"Found existing connection for player {player_id}, removing it"
                        )
                        if (
                            existing_ws != websocket
                        ):  # Don't remove the one we're adding
                            try:
                                ...
                                logging.warning(
                                    f"Removed old WebSocket {id(existing_ws)} for player {player_id}"
                                )
                            except Exception as e:
                                logging.error(
                                    f"Error removing old connection: {str(e)}"
                                )
            ...
            try:
                ...
                logging.warning(
                    f"Stored game_id {game_id} for WebSocket connection {id(websocket)}"
                )
                ...
                logging.warning(
                    f"Connection registration complete. Stats: "
    ...
    async def broadcast_to_game(self, game_id: str, message: dict):
        ...
        async with self.lock:
            if game_id not in self.active_connections:
                logging.debug(
                    f"Cannot broadcast to game {game_id}: no active connections"
                )
                return
            ...
            logging.debug(
                f"Broadcasting to {connection_count} connection(s) in game {game_id}"
            )
    ...
    async def send_personal_message(self, websocket: WebSocket, message: dict):
        ...
        async with self.lock:
            ...
            if websocket not in self.socket_player_map:
                logging.warning(
                    f"Cannot send message: WebSocket {id(websocket)} not found in player map"
                )
                return
            ...
            if websocket in self.socket_game_map:
                ...
                if found_game_id in self.active_connections:
                    if websocket in self.active_connections[found_game_id]:
                        ...
                        logging.debug(
                            f"Found valid connection for player {player_id} in game {found_game_id}"
                        )
            ...
            if not websocket_valid:
                logging.error(
                    f"DATA CONSISTENCY ERROR: WebSocket {id(websocket)} for player {player_id} "
                ...
                logging.error(
                    f"Connection maps state: active games: {active_games}, "
        ...
        if msg_type not in ["pong"]:
            logging.debug(
                f"Sending personal message of type '{msg_type}' to player {player_id}"
            )
        ...
        try:
            ...
            if not isinstance(message, dict):
                ...
                message = {
                    "type": "error",
                    "data": {"message": "Internal error: Invalid message format"},
                }

            # Serialize message preserving unicode characters
            json_message = json.dumps(message, ensure_ascii=False)
            message_preview = json_message[:100] + (
                "..." if len(json_message) > 100 else ""
            )
        ...
        try:
            ...
            if (
                ...
                logging.warning(
                    f"WebSocket for player {player_id} is in DISCONNECTED state, cannot send"
                )
            ...
            logging.debug(
                f"Successfully sent message of type {msg_type} to {player_id}"
            )
        except RuntimeError as e:
            # Common runtime errors from FastAPI WebSockets
            if "disconnected" in str(e) or "closed" in str(e):
                logging.warning(
                    f"WebSocket for player {player_id} is already disconnected: {str(e)}"
                )
            else:
    ...
    async def send_to_player(
        self, game_id: str, player_id: str, message: dict, max_retries: int = 2
    ) -> bool:
        """
        ...
        async with self.lock:
            if game_id not in self.active_connections:
                logging.warning(
                    f"Cannot send to player {player_id} in game {game_id}: game not found"
                )
                return False
        ...
        if not player_connections:
            logging.warning(
                f"No connections found for player {player_id} in game {game_id}"
            )
            # We'll retry later if needed
        else:
            logging.warning(
                f"Found {len(player_connections)} connection(s) for player {player_id}"
            )
        ...
        for retry in range(max_retries + 1):  # +1 for initial attempt
            ...
            if not player_connections and retry > 0:
                async with self.lock:
                    logging.warning(
                        f"Retry {retry}/{max_retries}: Refreshing connection map for player {player_id}"
                    )
                    logging.warning(
                        f"Connection map state: game_id={game_id}, player_id={player_id}"
                    )

                    if game_id in self.active_connections:
                        logging.warning(
                            f"  - connections count for game: {len(self.active_connections[game_id])}"
                        )
                        for conn in self.active_connections[game_id]:
                            mapped_player = self.socket_player_map.get(conn, "unknown")
                            logging.warning(
                                f"    - connection {id(conn)} maps to player {mapped_player}"
                            )
                            if mapped_player == player_id:
                                player_connections.append(conn)
                                logging.warning(
                                    f"    - added connection {id(conn)} to player_connections"
                                )
            ...
            for connection in player_connections:
                try:
                    if retry == 0:
                        logging.warning(
                            f"Sending message of type '{message.get('type')}' to player {player_id}"
                        )
                    else:
                        logging.warning(
                            f"Retry {retry}/{max_retries}: Sending message of type '{message.get('type')}' to player {player_id}"
                        )
                    ...
                    logging.error(
                        f"Unexpected error sending to player {player_id}: {str(e)}"
                    )
                    disconnected.append(connection)
            ...
            if not message_sent and retry < max_retries:
                logging.warning(
                    f"Retry {retry+1}/{max_retries}: Message not sent to player {player_id}, waiting before retry"
                )
                # Wait a bit before retrying
                await asyncio.sleep(1.0)
            elif not message_sent:
                logging.warning(
                    f"Failed to send message to player {player_id} in game {game_id} after {max_retries} retries"
                )
...
class GameStateNotifier:
    ...
    async def notify_new_round(
        self, game_id: str, round_name: str, community_cards: list
    ):
        """
    ...
    async def notify_round_bets_finalized(
        self, game_id: str, player_bets: list, current_pot: int
    ):
        """
    ...
    async def notify_pot_winners_determined(self, game_id: str, pots: list):
        ...
        message = {
            ...
            "data": {"pots": pots, "timestamp": datetime.now().isoformat()},
        }
        await self.connection_manager.broadcast_to_game(game_id, message)
    ...
    async def notify_chips_distributed_to_winners(self, game_id: str, game: PokerGame):
        ...
        message = {"type": "chips_distributed", "data": model.dict()}
        await self.connection_manager.broadcast_to_game(game_id, message)
    ...
    async def notify_game_update(
        self, game_id: str, game: PokerGame, game_to_model_func=None
    ):
        """
        ...
        player_connections = await self.connection_manager.get_player_connections(
            game_id
        )
        ...
        try:
            ...
            logging.warning(
                f"Generated game state for game {game_id} with {len(game_state.players)} players"
            )

            # Log a summary of the game state for debugging
            player_summary = ", ".join(
                [f"{p.name}({p.status})" for p in game_state.players]
            )
            ...
            try:
                ...
                logging.error(
                    f"Error serializing game state to JSON: {str(json_error)}"
                )
                logging.error(traceback.format_exc())
                return
        ...
        for player_id, player_sockets in player_connections.items():
            ...
            try:
                ...
                player_name = next(
                    (p.name for p in game_state.players if p.player_id == player_id),
                    player_id,
                )
                ...
                message = {"type": "game_state", "data": state_dict}
                ...
                for socket in player_sockets:
                    if socket in processed_sockets:
                        logging.warning(
                            f"Skipping already processed socket for player {player_name}"
                        )
                        continue
                    ...
                    try:
                        ...
                        logging.error(
                            f"Error sending to player {player_name}: {str(e)}"
                        )
                        logging.error(traceback.format_exc())
            except Exception as e:
                logging.error(
                    f"Error preparing game state message for player {player_name}: {str(e)}"
                )
                logging.error(traceback.format_exc())
        ...
        observer_connections = [
            conn for conn in connections if conn not in processed_sockets
        ]
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        logging.error(
                            f"Error serializing observer game state to JSON: {str(json_error)}"
                        )
                        logging.error(traceback.format_exc())
                        return
                    ...
                    logging.warning(
                        f"Sending observer game state to {len(observer_connections)} connections"
                    )
    ...
    async def notify_player_action(
        ...
        log_message = {
            "type": "action_log",
            "data": {"text": log_text, "timestamp": datetime.now().isoformat()},
        }
    ...
    async def notify_hand_result(self, game_id: str, game: PokerGame):
        ...
        logging.info(
            f"Notifying hand result for game {game_id}, hand {game.current_hand_id}"
        )
        ...
        if (
            result_message["data"].get("winners")
            and len(result_message["data"]["winners"]) > 0
        ):
    ...
    async def notify_action_request(self, game_id: str, game: PokerGame):
        ...
        logging.info(
            f"[ACTION_REQUEST-{execution_id}] Action request: {len(active_players)} active players, current_idx={game.current_player_idx}"
        )
        logging.info(
            f"[ACTION_REQUEST-{execution_id}] Current to_act set: {game.to_act}"
        )

        # Safety check for valid index
        if not active_players or game.current_player_idx >= len(game.players):
            logging.error(
                f"[ACTION_REQUEST-{execution_id}] Invalid current player index: {game.current_player_idx}, players: {len(game.players)}"
            )
            return
        ...
        logging.info(
            f"[ACTION_REQUEST-{execution_id}] Initial current player: {current_player.name} (ID: {current_player.player_id})"
        )
        logging.info(
            f"[ACTION_REQUEST-{execution_id}] Current player status: {current_player.status.name}, in to_act: {current_player.player_id in game.to_act}"
        )

        # Verify this player is actually active and needs to act
        if (
            current_player.status != PlayerStatus.ACTIVE
            or current_player.player_id not in game.to_act
        ):
            logging.error(
                f"[ACTION_REQUEST-{execution_id}] Current player {current_player.name} cannot act: status={current_player.status.name}, in to_act={current_player.player_id in game.to_act}"
            )
            ...
            for p in active_players:
                if p.player_id in game.to_act:
                    logging.info(
                        f"[ACTION_REQUEST-{execution_id}] Found alternative active player who can act: {p.name}"
                    )
                    current_player = p
            ...
            if not found_alternative:
                logging.error(
                    f"[ACTION_REQUEST-{execution_id}] No active players who can act found"
                )
                return
            ...
            try:
                ...
                logging.info(
                    f"[ACTION_REQUEST-{execution_id}] Updating current_player_idx from {game.current_player_idx} to {alt_idx}"
                )
                game.current_player_idx = alt_idx
            except ValueError:
                logging.error(
                    f"[ACTION_REQUEST-{execution_id}] Could not find alternative player in players list"
                )
                return
        ...
        try:
            ...
            if game_obj:
                ...
                player = next(
                    (p for p in game_obj.players if p.id == current_player.player_id),
                    None,
                )
        ...
        for retry in range(3):  # Try up to 3 times
            try:
                if retry > 0:
                    ...
                    logging.warning(
                        f"Retry {retry}/3: Sending action request to player {current_player.player_id}"
                    )
                ...
                if success:
                    logging.warning(
                        f"Successfully sent action request to player {current_player.player_id}"
                    )
                    break  # Exit retry loop if successful
                else:
                    # If not successful even with retries, try manual reconnection
                    logging.warning(
                        f"Failed to send action request, attempting connection recovery"
                    )

            except Exception as retry_error:
                logging.error(
                    f"Error sending action request (attempt {retry+1}/3): {str(retry_error)}"
                )
