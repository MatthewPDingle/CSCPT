...
class HandEvaluator:
    ...
    def evaluate(cards: Set[Card]) -> Tuple[HandRank, List[int]]:
        ...
        if flush_cards and straight_high:
            flush_ranks = sorted(
                [card.rank.value for card in flush_cards], reverse=True
            )
        ...
        if royal_flush:
            ...
            return HandRank.FULL_HOUSE, [
                triplets[0],
                pairs[0] if len(pairs) > 0 else triplets[1],
            ]
        elif flush_suit:
            return (
                HandRank.FLUSH,
                sorted([card.rank.value for card in flush_cards], reverse=True)[:5],
            )
        elif straight_high:
