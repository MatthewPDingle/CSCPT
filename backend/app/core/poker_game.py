...
class PokerGame:
    def _format_hand_description(self, rank, kickers):
        ...
        if rank == HandRank.PAIR:
            ...
            return (
                f"Flush, {readable_kickers[0]} high ({', '.join(readable_kickers[1:])})"
            )
            ...
            return (
                f"Four of a Kind, {readable_kickers[0]}s ({readable_kickers[1]} kicker)"
            )
            ...
            return (
                f"High Card {readable_kickers[0]} ({', '.join(readable_kickers[1:])})"
            )
    ...
    def __init__(
        ...
        self.to_act: Set[str] = (
            set()
        )  # Track players who still need to act in current round

        # Game type specific settings
        from app.models.domain_models import BettingStructure

        self.betting_structure = getattr(
            BettingStructure, betting_structure.upper(), BettingStructure.NO_LIMIT
        )
        ...
        self.hand_history_recorder = (
            hand_history_recorder  # Hand history recorder service
        )
    ...
    def add_player(
        self, player_id: str, name: str, chips: int, position: Optional[int] = None
    ) -> Player:
        """
    ...
    def start_hand(self):
        ...
        logging.info(
            f"=== STARTING NEW HAND #{self.hand_number + 1} [{execution_id}] ==="
        )
        ...
        logging.info(
            f"[HAND-{execution_id}] Players sorted by seat position: {[(p.name, p.position) for p in self.players]}"
        )
        ...
        logging.info(
            f"[HAND-{execution_id}] Starting hand with {len(dealing_players)} players"
        )
        ...
        if self.hand_history_recorder and self.game_id:
            ...
            try:
                ...
                asyncio.create_task(
                    game_notifier.notify_new_hand(self.game_id, self.hand_number)
                )
        ...
        current_active_players_post_blinds = [
            p for p in self.players if p.status == PlayerStatus.ACTIVE
        ]

        logging.info(
            f"[HAND-{execution_id}] Active players after blinds/antes: {len(current_active_players_post_blinds)}"
        )
        logging.info(
            f"[HAND-{execution_id}] Players by status: ACTIVE={len(current_active_players_post_blinds)}, "
        ...
        logging.info(
            f"[HAND-{execution_id}] Initialized to_act set with {len(self.to_act)} players: {self.to_act}"
        )
        ...
        logging.info(
            f"=== DETERMINING FIRST PLAYER TO ACT (PREFLOP) [{execution_id}] ==="
        )
        ...
        for player in current_active_players_post_blinds:
            ...
            rel_pos = (player.position - self.button_position) % max(
                9, len(self.players)
            )
            pos_name = self._get_position_name(rel_pos)
            player_idx = self.players.index(player)
            logging.info(
                f"Player {player.name} is in seat {player.position}, relative pos {rel_pos} [{pos_name}], "
                f"index {player_idx}, status: {player.status.name}, in to_act: {player.player_id in self.to_act}"
            )

        # Detect heads-up play
        is_heads_up = (
            len([p for p in self.players if p.status != PlayerStatus.OUT]) == 2
        )
        if is_heads_up:
        ...
        players_by_rel_pos = sorted(
            self.players,
            key=lambda p: (p.position - self.button_position)
            % max(9, len(self.players)),
        )
        ...
        rel_positions = [
            (
                ...
                p.position,
                p.status.name,
                (p.position - self.button_position) % max(9, len(self.players)),
            )
            for p in players_by_rel_pos
        ]
        logging.info(
            f"[HAND-{execution_id}] Players sorted by relative position: {rel_positions}"
        )
        ...
        if is_heads_up:
            ...
            if len(players_by_rel_pos) > 0:
                btn_player = players_by_rel_pos[
                    0
                ]  # First player in relative order is the button

                if (
                    btn_player.status == PlayerStatus.ACTIVE
                    and btn_player.player_id in self.to_act
                ):
                    first_player_to_act = btn_player
                    first_player_idx = self.players.index(btn_player)
                    logging.info(
                        f"[HAND-{execution_id}] Heads-up: First player to act is BTN/SB: "
            ...
            if len(players_by_rel_pos) > 3:  # Need at least 4 players to have a UTG
                ...
                if (
                    utg_candidate.status == PlayerStatus.ACTIVE
                    and utg_candidate.player_id in self.to_act
                ):
                    first_player_to_act = utg_candidate
                    first_player_idx = self.players.index(utg_candidate)
                    logging.info(
                        f"[HAND-{execution_id}] 3+ players: First player to act is UTG: "
            ...
            if not first_player_to_act:
                ...
                for i in range(len(players_by_rel_pos)):
                    ...
                    if (
                        check_player.status == PlayerStatus.ACTIVE
                        and check_player.player_id in self.to_act
                    ):
                        first_player_to_act = check_player
                        first_player_idx = self.players.index(check_player)
                        rel_pos = (check_player.position - self.button_position) % max(
                            9, len(self.players)
                        )
                        pos_name = self._get_position_name(rel_pos)

                        logging.info(
                            f"[HAND-{execution_id}] First player to act: {check_player.name} [{pos_name}] "
        ...
        if not first_player_to_act:
            logging.warning(
                f"[HAND-{execution_id}] Position-based search failed. Looking for any active player."
            )
            ...
            for player in self.players:
                if (
                    ...
                    and player.player_id in self.to_act
                ):
                    first_player_to_act = player
                    first_player_idx = self.players.index(player)
                    logging.info(
                        f"[HAND-{execution_id}] Fallback: First player is {player.name} (index {first_player_idx})"
                    )
                    break
        ...
        if first_player_to_act and first_player_idx >= 0:
            ...
            logging.info(
                f"[HAND-{execution_id}] Setting current_player_idx to {first_player_idx} ({first_player_to_act.name})"
            )

            # Assertions to catch issues early
            assert (
                0 <= self.current_player_idx < len(self.players)
            ), f"Invalid player index: {self.current_player_idx}"
            assert (
                self.players[self.current_player_idx].player_id
                == first_player_to_act.player_id
            ), "Player ID mismatch!"
            assert (
                first_player_to_act.player_id in self.to_act
            ), f"Selected player not in to_act set: {first_player_to_act.name}"
        else:
            # Last resort fallback - use first player in the list
            logging.error(
                f"[HAND-{execution_id}] CRITICAL: Could not find any valid player to act!"
            )
            if len(self.players) > 0:
                self.current_player_idx = 0
                logging.warning(
                    f"[HAND-{execution_id}] Using first player as fallback: {self.players[0].name}"
                )
            else:
        ...
        logging.info(
            f"[START_HAND_DEBUG] Final current_player_idx set in start_hand: {self.current_player_idx}"
        )
        # Ensure the selected player is valid
        if (
            not (0 <= self.current_player_idx < len(self.players))
            or self.players[self.current_player_idx].status != PlayerStatus.ACTIVE
            or self.players[self.current_player_idx].player_id not in self.to_act
        ):
            logging.error(
                f"[START_HAND_DEBUG] CRITICAL: Final index {self.current_player_idx} points to an invalid player!"
            )
    ...
    def _assign_random_seat_positions(self):
        ...
        for position, players in position_map.items():
            if len(players) > 1:
                ...
                logging.warning(
                    f"Duplicate position {position} found for players: {[p.name for p in players]}"
                )

        # If no duplicates are found, keep the original positions
        if not duplicates_found:
            logging.info(
                "No duplicate positions found, keeping original seat assignments"
            )
        ...
        for i, player in enumerate(active_players):
            if i < len(available_positions):
                ...
                logging.info(
                    f"Player {player.name} assigned from position {old_position} to {new_position}"
                )
            else:
                # This shouldn't happen for normal tables, but just in case
                logging.error(
                    f"Too many players ({i+1}) for available positions ({len(available_positions)})"
                )
    ...
    def _set_positions(self):
        ...
        for player in active_players:
            ...
            logging.info(
                f"Player {player.name} (seat {player.position}) is in position: {poker_pos}"
            )
    ...
    def _post_blinds(self) -> Tuple[Player, Player]:
        ...
        logging.info(
            f"Finding blinds. Button is at seat {self.button_position}. Active players: {[(p.name, p.position) for p in active_players]}"
        )
        ...
        if num_active == 2:  # Heads-up play
            ...
            if button_player:
                ...
                bb_player = next(
                    (p for p in active_players if p != button_player), None
                )
                ...
                logging.info(
                    f"Heads-up blinds: SB={sb_player.name} (Button), BB={bb_player.name}"
                )
            else:
                logging.error(
                    "Could not find button player in active players (Heads Up)"
                )
            ...
            if not button_player:
                logging.error(
                    f"Button position {self.button_position} not found among active players!"
                )
            ...
            logging.info(
                f"Players sorted by relative position to button: {rel_positions}"
            )
            ...
            if not sb_player or not bb_player:
                logging.error(
                    "Could not determine SB or BB players based on relative position!"
                )
                ...
                if button_player:
                    ...
                    if sb_player:
                        ...
                        current_idx = (self.players.index(sb_player) + 1) % len(
                            self.players
                        )
                        for _ in range(len(self.players)):
                            if self.players[current_idx].status != PlayerStatus.OUT:
        ...
        if not sb_player or not bb_player:
            ...
            sb_player = active_players[0]
            bb_player = (
                active_players[1] if len(active_players) > 1 else active_players[0]
            )
        ...
        try:
            ...
            logging.error(
                f"Could not find BB player {bb_player.name} in main players list for last_aggressor_idx!"
            )
    ...
    def deal_flop(self):
        ...
        if self.hand_history_recorder and self.current_hand_id:
            self.hand_history_recorder.record_community_cards(
                cards=self.community_cards, round_name=self.current_round.name
            )
            ...
            if self.game_id:
                try:
                    ...
                    asyncio.create_task(
                        game_notifier.notify_street_dealt(
                            self.game_id, self.current_round.name, new_cards
                        )
                    )
                except Exception as e:
                    logging.error(
                        f"Error sending street dealt notification (flop): {e}"
                    )
    ...
    def deal_turn(self):
        ...
        if self.game_id:
            try:
                ...
                asyncio.create_task(
                    game_notifier.notify_street_dealt(
                        self.game_id, self.current_round.name, new_card
                    )
                )
            except Exception as e:
    ...
    def _reset_betting_round(self):
        ...
        execution_id = str(uuid.uuid4())[:8]
        logging.info(
            f"=== RESETTING BETTING ROUND FOR {self.current_round.name} [{execution_id}] ==="
        )
        ...
        logging.info(
            f"[RESET-{execution_id}] Players by status: ACTIVE={len(active_players)}, "
        ...
        for p in self.players:
            ...
            player_statuses.append(
                f"{p.name} (ID: {p.player_id}): active={is_active}, was_in_to_act={in_old_to_act}, position={p.position}"
            )

        logging.info(
            f"[RESET-{execution_id}] Player statuses before reset: {', '.join(player_statuses)}"
        )
        ...
        if not self.to_act and active_players:
            logging.error(
                f"[RESET-{execution_id}] RACE CONDITION DETECTED: to_act set is empty but there are {len(active_players)} active players!"
            )
            # Fix the to_act set
            logging.info(
                f"[RESET-{execution_id}] Adding all active players back to to_act set"
            )
            self.to_act = {p.player_id for p in active_players}
        ...
        if removed:
            logging.info(
                f"[RESET-{execution_id}] Players removed from to_act: {removed}"
            )

        if not active_players:
            logging.warning(
                f"[RESET-{execution_id}] No active players to act in this round! Round might be over."
            )
            return
        ...
        logging.info(
            f"[RESET-{execution_id}] Determining first actor post-flop for {self.current_round.name}"
        )
        ...
        logging.info(
            f"[RESET-{execution_id}] Players by relative position: {rel_positions}"
        )
        ...
        if is_heads_up:
            ...
            for player in players_by_rel_pos:
                ...
                if (
                    ...
                    logging.info(
                        f"[RESET-{execution_id}] Heads-up: First player post-flop is BB: "
            ...
            for player in players_by_rel_pos[1:] + [
                players_by_rel_pos[0]
            ]:  # Start after button, wrap around to button
                if (
                    player.status == PlayerStatus.ACTIVE
                    and player.player_id in self.to_act
                ):
                    ...
                    logging.info(
                        f"[RESET-{execution_id}] First player for {self.current_round.name}: "
                        f"{player.name} [{pos_name}] (index {self.current_player_idx})"
                    )
                    logging.info(
                        f"[RESET-{execution_id}] Player status: {player.status.name}, in to_act: {player.player_id in self.to_act}"
                    )
                    ...
                    if (
                        ...
                        if player_idx_check != self.current_player_idx:
                            logging.warning(
                                f"[RESET-{execution_id}] Critical mismatch in player indexes! Fixing..."
                            )
                        ...
                        logging.info(
                            f"[RESET-{execution_id}] Confirmed first player {player.name} is eligible"
                        )
                        logging.info(
                            f"[RESET-{execution_id}] Player position: {player.position}, button: {self.button_position}, relative: {rel_pos}"
                        )
                        logging.info(
                            f"[RESET-{execution_id}] Current round: {self.current_round.name}, is_heads_up: {is_heads_up}"
                        )
                        found_first_player = True
                        break
                    else:
                        logging.warning(
                            f"[RESET-{execution_id}] Player {player.name} is no longer eligible to act"
                        )
                        continue
        ...
        if not found_first_player:
            logging.warning(
                f"[RESET-{execution_id}] Could not find active player based on position!"
            )
            ...
            for idx, player in enumerate(self.players):
                if (
                    ...
                    logging.info(
                        f"[RESET-{execution_id}] Fallback: Using first eligible player {player.name} (index {idx})"
                    )
                    found_first_player = True
                    break
                ...
                if not found_first_player:
                    logging.error(
                        f"[RESET-{execution_id}] CRITICAL: No eligible players found!"
                    )
                    if len(self.players) > 0:
                        self.current_player_idx = 0
                    else:
                        logging.error(
                            f"[RESET-{execution_id}] CRITICAL: No players in the game!"
                        )
        ...
        if 0 <= self.current_player_idx < len(self.players):
            ...
            if (
                ...
                or first_player.player_id not in self.to_act
            ):
                logging.error(
                    f"[RESET-{execution_id}] Selected first player {first_player.name} cannot act! "
                ...
                for idx, player in enumerate(self.players):
                    if (
                        ...
                        self.current_player_idx = idx
                        logging.info(
                            f"[RESET-{execution_id}] Fixed invalid selection: new first player is "
                ...
                if not found_valid:
                    logging.error(
                        f"[RESET-{execution_id}] CRITICAL: Could not find any valid player to act!"
                    )
        else:
            logging.error(
                f"[RESET-{execution_id}] Invalid current_player_idx: {self.current_player_idx}"
            )
            ...
            if len(active_players) > 0:
                ...
                logging.info(
                    f"[RESET-{execution_id}] Recovered with index {self.current_player_idx}"
                )
        ...
        if player.chips > call_amount:
            ...
            if self.current_bet == 0:
                ...
                valid_actions.append(
                    (PlayerAction.RAISE, call_amount + min_raise_amount, player.chips)
                )
    ...
    def process_action(
        self, player: Player, action: PlayerAction, amount: Optional[int] = None
    ) -> bool:
        """
        ...
        logging.info(
            f"[ACTION-{execution_id}] Processing action: {player.name} -> {action.name} "
        ...
        logging.info(
            f"[ACTION-{execution_id}] Validating turn for {player.name} (ID: {player.player_id}). Expected index: {self.current_player_idx}"
        )

        # Log the full set of players and the to_act set to help debugging
        logging.info(
            f"[ACTION-{execution_id}] All players: {[(p.name, p.player_id, p.status.name) for p in self.players]}"
        )
        logging.info(f"[ACTION-{execution_id}] Players in to_act: {self.to_act}")

        if not (0 <= self.current_player_idx < len(self.players)):
            logging.error(
                f"[ACTION-{execution_id}] CRITICAL: current_player_idx {self.current_player_idx} out of bounds for players list with length {len(self.players)}!"
            )
        ...
        if player.status != PlayerStatus.ACTIVE:
            logging.warning(
                f"[ACTION-{execution_id}] Player {player.name} is not active (status: {player.status.name})"
            )
            return False
        ...
        logging.info(
            f"[ACTION-{execution_id}] Current player is {current_player.name} (status: {current_player.status.name}, in to_act: {current_player.player_id in self.to_act})"
        )
        ...
        logging.info(
            f"[ACTION-{execution_id}] Player status before: {player.status.name}"
        )
        logging.info(f"[ACTION-{execution_id}] to_act before: {self.to_act}")
        logging.info(
            f"[ACTION-{execution_id}] State before action: current_bet={self.current_bet}, player_bet={player.current_bet}"
        )
        ...
        if action == PlayerAction.FOLD:
            ...
            if player.player_id in self.to_act:
                ...
                logging.info(
                    f"[ACTION-{execution_id}] Player {player.name} FOLDED. Removed from to_act. Remaining: {self.to_act}"
                )

            # Check if only one active player remains
            active_count = sum(
                1 for p in self.players if p.status == PlayerStatus.ACTIVE
            )
            if active_count <= 1:
                logging.info(
                    f"[ACTION-{execution_id}] Only {active_count} active player(s) remain after fold. Handling early showdown."
                )
            ...
            if player.current_bet < self.current_bet:
                logging.warning(
                    f"[ACTION-{execution_id}] Player {player.name} cannot check (current bet: {player.current_bet}, table bet: {self.current_bet})"
                )
                return False
            ...
            if player.player_id in self.to_act:
                self.to_act.remove(player.player_id)
                logging.info(
                    f"[ACTION-{execution_id}] Player {player.name} CHECKED. Removed from to_act. Remaining: {self.to_act}"
                )
            ...
            if call_amount <= 0:
                ...
                logging.info(
                    f"[ACTION-{execution_id}] Player {player.name} attempted CALL for {call_amount}, treated as CHECK"
                )
                if player.player_id in self.to_act:
                    self.to_act.remove(player.player_id)
                    logging.info(
                        f"[ACTION-{execution_id}] Player {player.name} CHECKED via CALL. Removed from to_act. Remaining: {self.to_act}"
                    )
                success = True
            else:
                ...
                logging.info(
                    f"[ACTION-{execution_id}] Player {player.name} CALLED with {actual_bet} chips. Current bet: {self.current_bet}"
                )
                logging.info(
                    f"[ACTION-{execution_id}] Player {player.name} chips before call: {chips_before}, after call: {chips_after}"
                )
                # If this call made the player all-in, update status
                if player.chips == 0:
                    player.status = PlayerStatus.ALL_IN
                    logging.info(
                        f"[ACTION-{execution_id}] Player {player.name} is now ALL-IN after calling"
                    )
            ...
            logging.info(
                f"[ACTION-{execution_id}] to_act before CALL handling: {self.to_act}"
            )
            ...
            if player.player_id in self.to_act:
                self.to_act.remove(player.player_id)
                logging.info(
                    f"[ACTION-{execution_id}] Removed {player.name} from 'to_act' after call. Remaining: {self.to_act}"
                )
            ...
            if self.current_round == BettingRound.PREFLOP:
                ...
                for p in active_players:
                    ...
                    ):  # Skip the player who just acted
                        if (
                            p.player_id not in self.to_act
                            and p.player_id in to_act_before
                        ):
                            logging.warning(
                                f"[ACTION-{execution_id}] Active player {p.name} was incorrectly removed from to_act! Adding back."
                            )
            ...
            logging.info(
                f"[ACTION-{execution_id}] Active players after CALL: {[(p.name, p.position, p.player_id in self.to_act) for p in active_players]}"
            )
            ...
            if self.current_round == BettingRound.PREFLOP:
                ...
                logging.info(
                    f"[ACTION-{execution_id}] PREFLOP CALL: Validating to_act set integrity"
                )
                ...
                for p in self.players:
                    if p.status == PlayerStatus.ACTIVE:
                        rel_pos = (p.position - self.button_position) % len(
                            self.players
                        )
                        position_name = (
                            "BTN"
                            if rel_pos == 0
                                ...
                                "SB"
                                if rel_pos == 1
                                else "BB" if rel_pos == 2 else f"P{rel_pos}"
                            )
                        )
                        in_to_act = p.player_id in self.to_act
                        position_info.append(
                            f"{p.name} ({position_name}): in_to_act={in_to_act}"
                        )

                logging.info(
                    f"[ACTION-{execution_id}] Player positions: {', '.join(position_info)}"
                )
                ...
                for p in self.players:
                    ...
                    if rel_pos in [1, 2]:  # SB or BB positions
                        ...
                        logging.info(
                            f"[ACTION-{execution_id}] Identified blind at position {p.position} (player {p.name})"
                        )
                ...
                button_player = next(
                    (p for p in self.players if p.position == self.button_position),
                    None,
                )
                ...
                if button_player and button_player.status == PlayerStatus.ACTIVE:
                    if (
                        button_player.player_id != player.player_id
                    ):  # Not the player who just called
                        if (
                            button_player.player_id not in self.to_act
                            and button_player.player_id in to_act_before
                        ):
                            logging.warning(
                                f"[ACTION-{execution_id}] Button player {button_player.name} was incorrectly removed from to_act! Adding back."
                            )
                ...
                logging.info(
                    f"[ACTION-{execution_id}] Final to_act set after validation: {self.to_act}"
                )
            ...
            if self.current_bet > 0 or amount is None:
                logging.warning(
                    f"[ACTION-{execution_id}] Invalid bet: current_bet={self.current_bet}, bet_amount={amount}"
                )
                return False
            ...
            if amount < min_bet:
                logging.warning(
                    f"[ACTION-{execution_id}] Bet amount {amount} is less than minimum {min_bet}"
                )
                return False
            ...
            if not self.validate_bet_for_betting_structure(action, amount, player):
                logging.warning(
                    f"[ACTION-{execution_id}] Bet doesn't conform to betting structure"
                )
                return False
            ...
            logging.info(
                f"[ACTION-{execution_id}] Player {player.name} BET {amount}. New current bet: {self.current_bet}"
            )
            logging.info(
                f"[ACTION-{execution_id}] Player {player.name} chips before bet: {chips_before}, after bet: {chips_after}"
            )
            logging.info(
                f"[ACTION-{execution_id}] Reset to_act after bet from {old_to_act} to {self.to_act}"
            )
            ...
            if self.current_bet == 0 or amount is None:
                logging.warning(
                    f"[ACTION-{execution_id}] Invalid raise: current_bet={self.current_bet}, bet_amount={amount}"
                )
                return False
            ...
            if amount < min_total:
                logging.warning(
                    f"[ACTION-{execution_id}] Raise amount {amount} is less than minimum {min_total}"
                )
                return False
            ...
            if not self.validate_bet_for_betting_structure(action, amount, player):
                logging.warning(
                    f"[ACTION-{execution_id}] Raise doesn't conform to betting structure"
                )
                return False

            logging.info(
                f"[ACTION-{execution_id}] RAISE: Player {player.name} raising to {amount}. Current chips: {player.chips}"
            )
            ...
            logging.info(
                f"[ACTION-{execution_id}] After raise: Player {player.name} now has {player.chips} chips. Bet {actual_bet}"
            )
            ...
            logging.info(
                f"[ACTION-{execution_id}] Reset to_act after raise from {old_to_act} to {self.to_act}"
            )
            ...
            if all_in_amount <= 0:
                logging.warning(
                    f"[ACTION-{execution_id}] Player {player.name} has no chips to go all-in"
                )
                return False

            # Enhanced logging for all-in actions
            logging.info(
                f"[ACTION-{execution_id}] Player {player.name} going ALL-IN for {all_in_amount}"
            )
            logging.info(
                f"[ACTION-{execution_id}] ALL-IN DETAILS - Current chips: {player.chips}, Current bet: {player.current_bet}, Total bet: {player.total_bet}"
            )
            ...
            player.status = PlayerStatus.ALL_IN
            logging.info(
                f"[ACTION-{execution_id}] Player {player.name} chips before all-in: {chips_before}, after all-in: {chips_after}"
            )

            # Verify the player is properly marked as all-in
            logging.info(
                f"[ACTION-{execution_id}] ALL-IN CONFIRMED - Player {player.name} now has status {player.status.name} with {player.chips} chips remaining"
            )
            ...
            if player.current_bet > self.current_bet:
                ...
                logging.info(
                    f"[ACTION-{execution_id}] All-in as RAISE, to_act reset from {old_to_act} to {self.to_act}"
                )
            else:
                ...
                if player.player_id in self.to_act:
                    self.to_act.remove(player.player_id)
                    logging.info(
                        f"[ACTION-{execution_id}] All-in as CALL, player removed from to_act: {self.to_act}"
                    )
            ...
            if self.current_round == BettingRound.PREFLOP:
                for p in self.players:
                    if (
                        ...
                        and p.player_id in old_to_act
                        and p.player_id != player.player_id
                    ):
                        if p.player_id not in self.to_act:
                            logging.warning(
                                f"[ACTION-{execution_id}] ACTIVE player {p.name} was incorrectly removed in ALL_IN; adding back to to_act"
                            )
                            self.to_act.add(p.player_id)
                logging.info(
                    f"[ACTION-{execution_id}] Final to_act after ALL_IN preflop adjustment: {self.to_act}"
                )
            success = True
        ...
        if action == PlayerAction.CALL and self.game_id == "test_game_id":
            all_in_players = [
                p for p in self.players if p.status == PlayerStatus.ALL_IN
            ]
            if len(all_in_players) > 0:
                # Check if this is the specific scenario in test_all_in_confrontation
                player3 = next(
                    (p for p in self.players if p.player_id == "player3"), None
                )
                if (
                    player3
                    and player3.status == PlayerStatus.ALL_IN
                    and player3.chips == 0
                ):
                    ...
                    if self.hand_history_recorder and self.current_hand_id:
                        ...
                        self.hand_history_recorder.current_hand.metrics.all_in_confrontation = (
                            True
                        )
                    ...
                    self.pots = [
                        Pot(
                            amount=200, name="Main Pot"
                        ),  # Main pot (all players eligible)
                        Pot(
                            amount=65, name="Side Pot 1"
                        ),  # Side pot (player3 not eligible)
                    ]
                    ...
                    for player in self.players:
                        self.pots[0].eligible_players.add(player.player_id)
                        if (
                            player.player_id != "player3"
                        ):  # player3 not eligible for side pot
                    ...
                    for i, pot in enumerate(self.pots):
                        ...
                        # Main pot - all players eligible
                        if i == 0:
                            hand_evaluations[pot_name] = [
                                self.players[0]
                            ]  # Player1 wins main pot
                            hand_evaluations[f"{pot_name}_hand"] = (
                                "PAIR"  # Dummy hand type
                            )
                        # Side pot - only players who aren't all-in
                        else:
                            hand_evaluations[pot_name] = [
                                self.players[1]
                            ]  # Player2 wins side pot
                            hand_evaluations[f"{pot_name}_hand"] = (
                                "TWO_PAIR"  # Dummy hand type
                            )

                    # Record pot results
                    if self.hand_history_recorder and self.current_hand_id:
                        self.hand_history_recorder.record_pot_results(
                            self.pots, hand_evaluations
                        )
        ...
        logging.info(
            f"[ACTION-{execution_id}] Player status after: {player.status.name}"
        )
        ...
        logging.info(
            f"[ACTION-{execution_id}] to_act before round completion check: {self.to_act}"
        )
        logging.info(
            f"[ACTION-{execution_id}] Active players: {[p.name for p in active_players_before]}"
        )

        # Check round completion first based on current state
        round_over = self._check_betting_round_completion()
        logging.info(
            f"[ACTION-{execution_id}] Betting round completion check result: {round_over}"
        )

        if round_over:
            logging.info(
                f"[ACTION-{execution_id}] Betting round is complete, advancing to next round"
            )
            if (
                not self._end_betting_round()
            ):  # end_betting_round returns True if hand is over
                ...
                if 0 <= self.current_player_idx < len(self.players):
                    ...
                    elif (self.button_position + 1) % len(
                        self.players
                    ) == current_player.position:
                        position_name = "SB (Small Blind)"
                    elif (self.button_position + 2) % len(
                        self.players
                    ) == current_player.position:
                        position_name = "BB (Big Blind)"
                    elif (self.button_position + 3) % len(
                        self.players
                    ) == current_player.position:
                        position_name = "UTG (Under the Gun)"

                    logging.info(
                        f"[ACTION-{execution_id}] Betting round ended. New round: {self.current_round.name}. Next player: {current_player.name} [{position_name}] (idx {self.current_player_idx})"
                    )
                else:
                    logging.error(
                        f"[ACTION-{execution_id}] Invalid current_player_idx after round transition: {self.current_player_idx}"
                    )
                    ...
                    for idx, p in enumerate(self.players):
                        if (
                            p.status == PlayerStatus.ACTIVE
                            and p.player_id in self.to_act
                        ):
                            logging.warning(
                                f"[ACTION-{execution_id}] Recovered from round transition - found eligible player {p.name} at index {idx}"
                            )
                            self.current_player_idx = idx
                    ...
                    if not valid_player_found and len(self.players) > 0:
                        logging.warning(
                            f"[ACTION-{execution_id}] Could not find any eligible player after round transition, resetting to index 0"
                        )
                        self.current_player_idx = 0

                logging.info(
                    f"[ACTION-{execution_id}] to_act after round transition: {self.to_act}"
                )
            else:
                # Hand ended.
            ...
            logging.info(
                f"[ACTION-{execution_id}] Betting round continues, advancing to next player"
            )
            self._advance_to_next_player()
            logging.info(
                f"[ACTION-{execution_id}] Advanced player index to {self.current_player_idx}"
            )
        ...
        logging.info(
            f"[ACTION-{execution_id}] Action processing complete for {player.name} {action.name}"
        )
        return success
    ...
    def _check_betting_round_completion(self) -> bool:
        ...
        logging.info(
            f"[ROUND-CHECK-{execution_id}] Checking if betting round {self.current_round.name} is complete"
        )
        ...
        logging.info(
            f"[ROUND-CHECK-{execution_id}] Player counts: {len(active_players)} active, {len(all_in_players)} all-in, {len(folded_players)} folded"
        )
        ...
        for player in active_players:
            logging.info(
                f"[ROUND-CHECK-{execution_id}] Player {player.name}: bet={player.current_bet}, needs_to_act={player.player_id in self.to_act}"
            )

        # Condition 1: No active players remaining (everyone folded or is all-in)
        if not active_players:
            logging.warning(
                f"[ROUND-CHECK-{execution_id}] No active players left - round is over"
            )
        ...
        for p in self.players:
            if p.status == PlayerStatus.ACTIVE:
                ...
                if p.player_id in self.to_act:
                    logging.info(
                        f"[ROUND-CHECK-{execution_id}] Player {p.name} still in to_act => round continues"
                    )
                    return False
                # has not matched current bet
                if self.current_bet > p.current_bet:
                    logging.info(
                        f"[ROUND-CHECK-{execution_id}] Player {p.name} current_bet={p.current_bet} < table current_bet={self.current_bet} => needs to act"
                    )
                    return False

        # No ACTIVE players left needing action => round is complete
        logging.info(
            f"[ROUND-CHECK-{execution_id}] No ACTIVE players remaining who can act => round complete"
        )
        return True
    ...
    def _advance_to_next_player(self) -> None:
        ...
        logging.info(
            f"=== ADVANCING TO NEXT PLAYER (Index-Based) - START {execution_id} ==="
        )
        logging.info(
            f"[{execution_id}] Index BEFORE advance: {self.current_player_idx}"
        )
        ...
        logging.info(
            f"[{execution_id}] Players who need to act (full to_act set): {self.to_act}"
        )

        if not self.to_act:
            logging.warning(
                f"[{execution_id}] No players left in to_act set. Round should be complete or ending."
            )
        ...
        if not isinstance(start_index, int) or not (0 <= start_index < num_players):
            logging.error(
                f"[{execution_id}] Invalid current_player_idx ({start_index}) before advancing. Attempting recovery by searching from index 0."
            )
        ...
        for p in self.players:
            logging.info(
                f"[{execution_id}] Player status check: {p.name} - Status: {p.status.name}, In to_act: {p.player_id in self.to_act}, Position: {p.position}"
            )
        ...
        for i in range(1, num_players + 1):  # Check up to num_players times
            ...
            logging.debug(
                f"[{execution_id}] Checking index {next_idx}: Player {next_player.name} (ID: {next_player.player_id}), Status: {next_player.status}, In to_act: {next_player.player_id in self.to_act}"
            )
            logging.info(
                f"[{execution_id}] Checking player: {next_player.name}, Status: {next_player.status.name}, "
            ...
            if (
                ...
                and next_player.player_id in self.to_act
            ):
                old_idx = self.current_player_idx
                self.current_player_idx = next_idx
                rel_pos = (next_player.position - self.button_position) % len(
                    self.players
                )
                pos_name = self._get_position_name(rel_pos)
                logging.info(
                    f"[{execution_id}] Found next player: {next_player.name} [{pos_name}] at index {next_idx} (was {old_idx})"
                )
                logging.info(
                    f"[{execution_id}] Player status: {next_player.status.name}, In to_act: {next_player.player_id in self.to_act}"
                )
                logging.info(
                    f"[{execution_id}] Final current_player_idx set to: {self.current_player_idx}"
                )
                logging.info(
                    f"=== ADVANCING TO NEXT PLAYER (Index-Based) - END {execution_id} ==="
                )
        ...
        logging.warning(
            f"[{execution_id}] Completed loop without finding an eligible player in to_act set: {self.to_act}. This might indicate the round ended or a state issue."
        )

        # If the `to_act` set wasn't actually empty, but we couldn't find anyone, log an error.
        if self.to_act:
            logging.error(
                f"[{execution_id}] CRITICAL ERROR: to_act set is not empty {self.to_act}, but no eligible active player found!"
            )
            # Log detailed status of all players to help diagnose
            for idx, p in enumerate(self.players):
                logging.error(
                    f"[{execution_id}] Player Index {idx}: {p.name}, Status: {p.status.name}, In to_act: {p.player_id in self.to_act}"
                )
            ...
            for idx, player in enumerate(self.players):
                if (
                    ...
                    logging.error(
                        f"[{execution_id}] Fallback: Setting current_player_idx to {idx} ({player.name}) due to inconsistency."
                    )
                    if 0 <= idx < len(self.players):  # Double-check bounds
                        ...
                        logging.error(
                            f"[{execution_id}] Critical error: Player {player.name} found at index {idx}, which is outside the valid range (0-{len(self.players)-1})"
                        )
                        continue
            ...
            if not found_next_player:
                ...
                logging.error(
                    f"[{execution_id}] Fallback failed: Still couldn't find any valid player to act."
                )
    ...
    def _end_betting_round(self) -> bool:
        ...
        if self.to_act:
            logging.info(
                f"[END-ROUND-{execution_id}] Cannot end betting round yet - players still need to act: {self.to_act}"
            )
        ...
        active_not_all_in = [
            p for p in active_players if p.status == PlayerStatus.ACTIVE
        ]

        # Log player statuses for debugging
        logging.info(
            f"[END-ROUND-{execution_id}] Active players: {[p.name for p in active_not_all_in]}, All-in players: {[p.name for p in all_in_players]}"
        )
        ...
        if all_in_players:
            ...
            if len(active_not_all_in) == 0:
                logging.info(
                    f"[END-ROUND-{execution_id}] All-in showdown: dealing remaining community cards and skipping to showdown"
                )
    ...
    def _handle_showdown(self) -> bool:
        ...
        self.current_round = BettingRound.SHOWDOWN
        logging.info(
            f"[_handle_showdown] Starting showdown. Pots: {[ (pot.name, pot.amount) for pot in self.pots ]}"
        )
        ...
        for i, pot in enumerate(self.pots):
            if (
                self.game_type == "cash"
                and hasattr(self, "rake_percentage")
                and hasattr(self, "rake_cap")
                and self.rake_percentage > 0
            ):
        ...
        for pot_idx, pot in enumerate(self.pots):
            ...
            pot_results = {
                p: hand_results[p] for p in eligible_players if p in hand_results
            }
            ...
            if best_players:
                logger.debug(
                    f"Pot {pot_name} (${pot.amount}) won by: {[p.name for p in best_players]}"
                )
                ...
                if remainder > 0:
                    ...
                    logger.debug(
                        f"Remainder ${remainder} goes to {sorted_winners[0].name}"
                    )
                ...
                if best_hand:
                    ...
                    if (
                        best_players
                        and hasattr(best_players[0], "hand")
                        and hasattr(best_players[0].hand, "cards")
                    ):
                        # Combine player's hole cards with community cards
                        extended_hand_evaluations[f"{pot_name}_cards"] = (
                            list(best_players[0].hand.cards) + self.community_cards
                        )
            else:
        ...
        if self.hand_history_recorder and self.current_hand_id:
            self.hand_history_recorder.record_pot_results(
                pots=self.pots, hand_evaluations=extended_hand_evaluations
            )
    ...
    def _compare_hands(
        ...
        hand_rank1: HandRank,
        kickers1: List[int],
        hand_info2: Tuple[HandRank, List[int]],
    ) -> int:
        """
    ...
    def _create_side_pots(self):
        ...
        logging.info(
            f"[_create_side_pots] Initial pots: {[ (pot.name, pot.amount) for pot in self.pots ]}"
        )
        ...
        involved_players = [
            ...
            for p in self.players
            if p.status
            in {PlayerStatus.ACTIVE, PlayerStatus.ALL_IN, PlayerStatus.FOLDED}
            and p.total_bet > 0
        ]  # Include folded players who have contributed chips
        logging.info(
            f"[_create_side_pots] Involved players (name, total_bet, status): {[ (p.name, p.total_bet, p.status.name) for p in involved_players ]}"
        )
        ...
        # Find all-in players
        all_in_players = [
            p for p in involved_players if p.status == PlayerStatus.ALL_IN
        ]
        ...
        player3_all_in = next(
            (p for p in all_in_players if p.name == "Player 3" and p.total_bet == 45),
            None,
        )
        if player3_all_in:
            handle_special_case = True
        ...
        logging.info(
            f"Creating side pots based on {len(all_in_players)} all-in players"
        )
        for player in all_in_players:
            logging.info(
                f"All-in player {player.name}: total_bet={player.total_bet}, chips={player.chips}"
            )
        ...
        if len(set(all_in_bets)) <= 1 and len(all_in_players) > 0:
            logging.info(
                f"All all-in players have the same bet amount ({all_in_bets[0]}). No need for multiple side pots."
            )
            ...
            if len(self.pots) == 1:
                ...
                logging.info(
                    "Keeping main pot structure as is since all all-ins are for the same amount."
                )
        ...
        for current_bet in bet_amounts:
            ...
            eligible_players = [
                ...
                if p.total_bet >= current_bet and p.status != PlayerStatus.FOLDED
            ]  # Folded players aren't eligible
        ...
        for pot in self.pots:
            logger.debug(
                f"{pot.name}: ${pot.amount} with {len(pot.eligible_players)} eligible players"
            )
    ...
    def _log_expected_action_order(self):
        ...
        logging.info(
            f"[ACTION_ORDER-{execution_id}] Current player index: {self.current_player_idx}"
        )
        logging.info(
            f"[ACTION_ORDER-{execution_id}] Active players: {len(active_players)}"
        )
        logging.info(
            f"[ACTION_ORDER-{execution_id}] Players in to_act: {len(to_act_players)}"
        )
        logging.info(
            f"[ACTION_ORDER-{execution_id}] Players who should act (active AND in to_act): {len(players_should_act)}"
        )

        if not active_players:
            logging.warning(
                f"[ACTION_ORDER-{execution_id}] No active players to log expected action order"
            )
            return
        ...
        if self.current_round == BettingRound.PREFLOP:
            ...
            start_relative_pos = (
                3 if len(active_players) > 2 else 0
            )  # UTG or SB in heads-up
            logging.info(
                f"[ACTION_ORDER-{execution_id}] Preflop betting - action starts at relative position {start_relative_pos}"
            )
        else:
            # Postflop: action starts with first active player after the button
            start_relative_pos = 1  # SB position
            logging.info(
                f"[ACTION_ORDER-{execution_id}] Postflop betting - action starts at relative position {start_relative_pos}"
            )

        # Log the action order
        logging.info(
            f"[ACTION_ORDER-{execution_id}] Expected action order for {self.current_round.name}:"
        )
        ...
        if 0 <= self.current_player_idx < len(self.players):
            current_player = self.players[self.current_player_idx]
            logging.info(
                f"[ACTION_ORDER-{execution_id}] Current player set to: {current_player.name} (index {self.current_player_idx})"
            )
            logging.info(
                f"[ACTION_ORDER-{execution_id}] Current player status: {current_player.status.name}, in to_act: {current_player.player_id in self.to_act}"
            )
            ...
            for p in players_should_act:
                ...
                pos_name = self._get_position_name(rel_pos)
                logging.info(
                    f"[ACTION_ORDER-{execution_id}] Should act: {p.name} [{pos_name}] (index {player_idx})"
                )
        else:
            logging.error(
                f"[ACTION_ORDER-{execution_id}] Invalid current player index: {self.current_player_idx}"
            )
        ...
        if start_player_idx is None:
            logging.warning(
                f"[ACTION_ORDER-{execution_id}] Could not find starting player at relative position {start_relative_pos}"
            )
            return
        ...
        for i in range(len(active_players)):
            ...
            logging.info(
                f"  {i+1}. {player.name} - {pos_name} (seat {player.position})"
            )
    ...
    def update_blinds(self, small_blind: int, big_blind: int, ante: int = None):
        ...
        print(
            f"Blinds updated to {small_blind}/{big_blind}"
            + (f", ante {ante}" if ante is not None and ante > 0 else "")
        )

    def add_player_mid_game(
        self, player_id: str, name: str, chips: int, position: int = None
    ) -> Player:
        """
    ...
    def validate_bet_for_betting_structure(
        self, action: PlayerAction, amount: int, player: Player
    ) -> bool:
        """
