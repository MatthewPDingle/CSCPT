...
from app.core.poker_game import (
    PlayerStatus as PokerPlayerStatus,
    BettingRound as PokerBettingRound,
)
from app.models.domain_models import (
    ...
    BettingRound,
    PlayerAction,
    PlayerStatus as DomainPlayerStatus,
    TournamentInfo,
    CashGameInfo,
    BlindLevel,
    HandHistory,
...
class GameService:
    ...
    def create_game(
        self, game_type: GameType, name: Optional[str] = None, **options
    ) -> Game:
        """
        ...
        if game_type == GameType.CASH:
            ...
            try:
                betting_structure = getattr(
                    BettingStructure, betting_structure_str.upper()
                )
    ...
    def create_cash_game(
        ...
        logging.warning(
            f"Creating cash game with min_buy_in_chips={min_buy_in_chips}, max_buy_in_chips={max_buy_in_chips}"
        )
        ...
        options = {
            ...
            "max_buy_in": max(
                max_buy_in_chips, 2000
            ),  # Ensure it's at least 2000 for tests
    ...
    def add_player(
        ...
        if game.type == GameType.CASH and game.status == GameStatus.ACTIVE:
            if game_id in self.poker_games:
                ...
                poker_game.add_player_mid_game(
                    player.id, player.name, player.chips, player.position
                )
    ...
    def add_player_to_cash_game(
        ...
        if buy_in < game.cash_game_info.min_buy_in:
            raise ValueError(
                f"Buy-in must be at least {game.cash_game_info.min_buy_in}"
            )
    ...
    def _start_game_internal(self, game_id: str, is_async: bool = False) -> Game:
        ...
        if poker_game:
            logging.info(
                f"Game {game_id} started. Adding detailed logging for player positions."
            )

            # Log the current state of the game
            active_players = [
                p for p in poker_game.players if p.status != PokerPlayerStatus.OUT
            ]
            ...
            if 0 <= poker_game.current_player_idx < len(poker_game.players):
                ...
                # *** ADD LOGGING HERE ***
                logging.info(
                    f"Check after start_hand: Current Player Index: {poker_game.current_player_idx}"
                )
                logging.info(
                    f"Check after start_hand: Player object at index: {current_player_poker.name}"
                )
                logging.info(
                    f"Check after start_hand: Contents of to_act: {poker_game.to_act}"
                )
                logging.info(
                    f"Check after start_hand: Does to_act contain {current_player_poker.player_id}? {current_player_poker.player_id in poker_game.to_act}"
                )
                # *** END ADDED LOGGING ***

                # Log the current player's info
                logging.info(
                    f"First player to act: {current_player_poker.name} (index {poker_game.current_player_idx})"
                )
                logging.info(
                    f"Player status: {current_player_poker.status.name if hasattr(current_player_poker, 'status') else 'Unknown'}"
                )
                logging.info(
                    f"Is in to_act set: {current_player_poker.player_id in poker_game.to_act}"
                )

                # Ensure player is active and needs to act before proceeding
                if (
                    current_player_poker.status == PokerPlayerStatus.ACTIVE
                    ...
                    if current_player_domain and is_async:
                        if current_player_domain.is_human:
                            logging.info(
                                f"Game started. First player is Human ({current_player_domain.name}). Waiting for action via WebSocket."
                            )
                            try:
                                ...
                                asyncio.create_task(
                                    game_notifier.notify_action_request(
                                        game.id, poker_game
                                    )
                                )
                            ...
                            logging.info(
                                f"Game started. First player is AI ({current_player_domain.name}), but we'll wait for WebSocket connection before triggering."
                            )
                            # AI action will be triggered after WebSocket connection in game_ws.py
                    else:
                        logging.warning(
                            f"Game started, but domain player for poker player {current_player_poker.player_id} not found or running in sync mode."
                        )
                else:
                    logging.warning(
                        f"Game started, but current player {current_player_poker.name} is not active or doesn't need to act."
                    )
                    logging.warning(
                        f"Status: {current_player_poker.status.name if hasattr(current_player_poker, 'status') else 'Unknown'}"
                    )
                    logging.warning(
                        f"In to_act: {current_player_poker.player_id in poker_game.to_act}"
                    )
                    ...
                    for idx, player in enumerate(poker_game.players):
                        if (
                            ...
                            and player.player_id in poker_game.to_act
                        ):
                            logging.info(
                                f"Found alternative first player to act: {player.name} (index {idx})"
                            )
                            poker_game.current_player_idx = idx
                            break
            else:
                logging.error(
                    f"Game started, but current player index {poker_game.current_player_idx} is out of bounds. Players: {len(poker_game.players)}"
                )
            ...
            logging.error(
                f"Game started, but poker game instance for game {game_id} not found."
            )
    ...
    def _start_new_hand(self, game: Game) -> Hand:
        ...
        if hand_number > 1:
            ...
            active_positions = sorted(
                [p.position for p in game.players if p.status != DomainPlayerStatus.OUT]
            )
        ...
        logging.info(
            f"Dealer position selected: {dealer_position} (hand #{hand_number})"
        )
        ...
        if game.type == GameType.CASH:
            ...
            if (
                ...
                and len(tournament.blind_structure) >= tournament.current_level
            ):
            ...
            # Otherwise use default progression logic
                ...
                tournament.ante_enabled
                and tournament.current_level >= tournament.ante_start_level
                and ante == 0
            ):
        ...
        utg_pos = (
            poker_game.button_position + 3
        ) % player_count  # UTG is 3 positions after button
        ...
        if 0 <= poker_game.current_player_idx < len(poker_game.players):
            ...
            logging.info(
                f"First to act: {current_player.name} (index {poker_game.current_player_idx})"
            )
    ...
    def advance_tournament_level(self, game_id: str) -> Game:
        ...
        if (
            ...
            small_blind = self._calculate_blind_for_level(
                tournament.starting_small_blind, level
            )
            big_blind = self._calculate_blind_for_level(
                tournament.starting_big_blind, level
            )
    ...
    async def _process_action_impl(
        ...
        logging.info(
            f"[PROCESS-ACTION-{execution_id}] Starting for player {player_id}, action {action}, amount {amount}"
        )
        ...
        async with game_lock:
            ...
            if game.status != GameStatus.ACTIVE:
                raise ValueError(
                    f"Cannot process action for game with status {game.status}"
                )
            ...
            if poker_game:
                ...
                logging.info(
                    f"Processing action: {player.name} [{position_name} - Seat {player.position}] {action} {amount if amount else ''}"
                )
                ...
                if 0 <= poker_game.current_player_idx < len(poker_game.players):
                    ...
                    logging.info(
                        f"Next player to act: {current_player.name} (index {poker_game.current_player_idx})"
                    )
            ...
            if game_id in self.poker_games:
                ...
                if poker_player:
                    ...
                    if poker_game.current_round == PokerBettingRound.SHOWDOWN:
                        ...
                        try:
                            ...
                            if len(board) == 3:
                                ...
                                await game_notifier.notify_street_dealt(
                                    game_id, PokerBettingRound.TURN.name, [board[3]]
                                )
                            ...
                            logging.error(
                                f"Error broadcasting showdown board cards: {e}"
                            )
                        ...
                        try:
                            ...
                            await game_notifier.notify_round_bets_finalized(
                                game_id, player_bets, total_pot
                            )
                        except Exception as e:
                            logging.error(
                                f"Error broadcasting round bets finalized: {e}"
                            )
                        ...
                        try:
                            ...
                            for pp in poker_game.players:
                                if (
                                    hasattr(pp, "hand")
                                    and pp.hand
                                    and pp.status != PokerPlayerStatus.FOLDED
                                ):
                                    player_hands.append(
                                        {
                            ...
                            await game_notifier.notify_showdown_hands_revealed(
                                game_id, player_hands
                            )
                        except Exception as e:
                        ...
                        try:
                            ...
                            await game_notifier.notify_pot_winners_determined(
                                game_id, pots_info
                            )
                        except Exception as e:
                        ...
                        try:
                            ...
                            for p in game.players:
                                ...
                                if pp:
                                    p.chips = pp.chips
                            self.game_repo.update(game)
                            await game_notifier.notify_chips_distributed_to_winners(
                                game_id, poker_game
                            )
                        except Exception as e:
                        ...
                        try:
                            ...
                            if pots_info and pots_info[0]["winners"]:
                                await game_notifier.notify_hand_visually_concluded(
                                    game_id
                                )
                        except Exception as e:
                    ...
                    for p in game.players:
                        ...
                        poker_p = next(
                            (pp for pp in poker_game.players if pp.player_id == p.id),
                            None,
                        )
                        if poker_p:
                            ...
                            p.status = status_map.get(
                                poker_p.status, DomainPlayerStatus.ACTIVE
                            )
    ...
    async def _request_and_process_ai_action(self, game_id: str, player_id: str):
        ...
        logging.info(
            f"[AI-ACTION-START-{execution_id}] Starting AI Action for Player {player_id} in Game {game_id}"
        )
        ...
        logging.info(
            f"[AI-ACTION-{execution_id}] --- Starting AI Action for Player {player_id} in Game {game_id} ---"
        )
        ...
        if len(call_stack) > 1:
            logging.info(
                f"[AI-ACTION-{execution_id}] AI action chain depth: {len(call_stack)} - This is a recursive call"
            )
        else:
            logging.info(
                f"[AI-ACTION-{execution_id}] AI action chain depth: 1 - This is the first call in the chain"
            )
        ...
        if not poker_game:
            logging.error(
                f"AI Action Aborted: PokerGame instance for {game_id} not found."
            )
            return
        ...
        active_poker_players = [
            p for p in poker_game.players if p.status == PokerPlayerStatus.ACTIVE
        ]
        ...
        logging.info(
            f"[AI-ACTION-CHECK-{execution_id}] Checking turn: current_idx={current_idx}, player_list_len={player_list_len}, target_player_id={target_player_id}"
        )
        ...
        if not active_poker_players:
            logging.error(
                f"AI Action Aborted: No active players. Context: current_idx={current_idx}, player_id={player_id}"
            )
            return
        elif not (0 <= current_idx < player_list_len):
            logging.error(
                f"AI Action Aborted: current_player_idx {current_idx} is out of bounds for players list length {player_list_len}. Context: player_id={player_id}"
            )
            return
        elif poker_game.players[current_idx].player_id != target_player_id:
            logging.error(
                f"AI Action Aborted: It's not player {target_player_id}'s turn. Current turn is index {current_idx} (Player ID: {current_turn_player_id})"
            )
            return
        # If we pass all checks, log success
        logging.info(
            f"[AI-ACTION-CHECK-{execution_id}] Turn validation passed for player {player_id}"
        )
        # *** END CRITICAL CHECK ***
        ...
        if poker_game.button_position == poker_player.position:
            position_name = "BTN (Dealer)"
        elif (poker_game.button_position + 1) % len(
            active_poker_players
        ) == poker_player.position:
            position_name = "SB (Small Blind)"
        elif (poker_game.button_position + 2) % len(
            active_poker_players
        ) == poker_player.position:
            position_name = "BB (Big Blind)"
        elif (poker_game.button_position + 3) % len(
            active_poker_players
        ) == poker_player.position:
            position_name = "UTG (Under the Gun)"

        logging.info(
            f"AI Action: Player {domain_player.name} [{position_name} - Position {poker_player.position}] (Player ID: {player_id}), Archetype: {domain_player.archetype or 'Default'}"
        )
        logging.info(
            f"AI Action: Current Round: {poker_game.current_round.name}, Current Bet: {poker_game.current_bet}, To Act: {poker_game.to_act}"
        )
        ...
        try:
            highest_total = max(
                p["total_bet"]
                for p in game_state_dict["players"]
        ...
        try:
            ...
            for attempt in range(1, max_retries + 1):
                logging.info(
                    f"[AI-ACTION-{execution_id}] Attempt {attempt}/{max_retries} for AI decision"
                )
                try:
                    ...
                    logging.info(
                        f"[AI-ACTION-{execution_id}] Attempt {attempt}: Raw AI decision: {ai_decision}"
                    )
                    action_str, amount, metadata = AgentResponseParser.parse_response(
                        ai_decision
                    )
                    logging.info(
                        f"[AI-ACTION-{execution_id}] Attempt {attempt}: Parsed action: {action_str}, amount: {amount}"
                    )
                    # Handle null or zero amount for ALL_IN
                    if action_str.lower().replace("-", "_").replace(
                        " ", "_"
                    ) == "all_in" and (amount is None or amount <= 0):
                        correct_amount = poker_player.chips + poker_player.current_bet
                        logging.warning(
                            f"[AI-ACTION-{execution_id}] Corrected ALL_IN amount for player {player_id} from {amount} to {correct_amount}"
                        )
                        amount = correct_amount
                    action_str, amount = AgentResponseParser.apply_game_rules(
                        action_str, amount, game_state_dict
                    )
                    logging.info(
                        f"[AI-ACTION-{execution_id}] Attempt {attempt}: Action after rules: {action_str}, amount: {amount}"
                    )
                    ...
                    if action_type == "ALL_IN":
                        final_action_amount = amount or (
                            poker_player.chips + poker_player.current_bet
                        )
                    else:
                    ...
                    logging.warning(
                        f"[AI-ACTION-{execution_id}] Attempt {attempt} failed: {e}"
                    )
                    if attempt == max_retries:
                        final_action_type = "FOLD"
                        final_action_amount = None
                        logging.error(
                            f"[AI-ACTION-{execution_id}] All {max_retries} attempts failed. Defaulting to FOLD."
                        )
            ...
            logging.info(
                f"[AI-ACTION-{execution_id}] Final chosen action for player {player_id}: {action_type} with amount {action_amount}"
            )
            ...
            try:
                ...
                logging.error(
                    f"Invalid action type '{action_type}' received from AI or mapping. Defaulting to FOLD."
                )
            ...
            logging.info(
                f"[AI-ACTION-{execution_id}] Acquiring game lock for processing action"
            )
            # Capture player's current bet to compute actual call size for 'CALL' actions
            prev_player_bet = poker_player.current_bet
            async with game_lock:
                logging.info(
                    f"[AI-ACTION-{execution_id}] Lock acquired - Processing {poker_action.name} {action_amount if action_amount is not None else ''} in PokerGame"
                )
                logging.info(
                    f"[AI-ACTION-{execution_id}] Before process_action - to_act: {poker_game.to_act}"
                )
                success = poker_game.process_action(
                    poker_player, poker_action, action_amount
                )
                logging.info(
                    f"[AI-ACTION-{execution_id}] After process_action - to_act: {poker_game.to_act}"
                )
                logging.info(
                    f"[AI-ACTION-{execution_id}] PokerGame process_action result: {success}"
                )
                ...
                if not success:
                    ...
                    logging.error(
                        f"[AI-ACTION-{execution_id}] Invalid AI action {poker_action.name} {action_amount}. Forcing FOLD to prevent stall."
                    )
                    # Force fold
            ...
            if success:
                ...
                action_history = ActionHistory(
                    ...
                    round=(
                        game.current_hand.current_round
                        if game.current_hand
                        else BettingRound.PREFLOP
                    ),
                )
                ...
                await game_notifier.notify_player_action(
                    ...
                    total_hand_bet=(
                        post_hand_bet
                        if (
                            poker_action == PokerPlayerAction.ALL_IN
                            or (
                                poker_action == PokerPlayerAction.CALL
                                and poker_player.chips == 0
                            )
                        )
                        else None
                    ),
                ...
                if poker_game.current_round == PokerBettingRound.SHOWDOWN:
                    ...
                    try:
                        ...
                        if len(board) == 3:
                            await game_notifier.notify_street_dealt(
                                game_id, PokerBettingRound.TURN.name, board[:4]
                            )
                            await asyncio.sleep(1.0)
                            await game_notifier.notify_street_dealt(
                                game_id, PokerBettingRound.RIVER.name, board
                            )
                            await asyncio.sleep(1.0)
                        elif len(board) == 4:
                            await game_notifier.notify_street_dealt(
                        ...
                        logging.error(
                            f"Error broadcasting post-flop showdown boards: {e}"
                        )
                    # ------------------------------------------------------------------
                    # 1) Collect the final street bets into the pot (frontend will animate)
                    try:
                        ...
                        logging.error(
                            f"Error broadcasting round bets finalized (AI path): {e}"
                        )
                    # End-of-Hand: Frontend-driven animation sequence via granular events
                    # ------------------------------------------------------------------
                    # 2) Reveal showdown hole cards
                    ...
                    try:
                        for p in game.players:
                            pp = next(
                                (x for x in poker_game.players if x.player_id == p.id),
                                None,
                            )
                            if pp:
                                p.chips = pp.chips
                    ...
                    logging.info(
                        f"AI Action: Moved button to position {poker_game.button_position}"
                    )
                    self._start_new_hand(game)
                    self.game_repo.update(game)
                    ...
                    if new_poker_game:
                        logging.info(
                            "AI Action: Notifying clients about the *new* hand state."
                        )
                        await game_notifier.notify_game_update(game_id, new_poker_game)
                        # Trigger first player action in new hand
                        if (
                            0
                            <= new_poker_game.current_player_idx
                            < len(new_poker_game.players)
                        ):
                            first = new_poker_game.players[
                                new_poker_game.current_player_idx
                            ]
                            first_domain = next(
                                (p for p in game.players if p.id == first.player_id),
                                None,
                            )
                            if first_domain and not first_domain.is_human:
                                asyncio.create_task(
                                    self._request_and_process_ai_action(
                                        game_id, first.player_id
                                    )
                                )
                            else:
                        ...
                        logging.error(
                            "AI Action: Could not find poker game instance after starting new hand to notify clients."
                        )
                else:
                    ...
                    logging.info(
                        f"[AI-ACTION-{execution_id}] Hand continues after AI action - turn already advanced internally"
                    )

                    # Log current state for monitoring
                    logging.info(
                        f"[AI-ACTION-{execution_id}] Current player_idx: {poker_game.current_player_idx}"
                    )
                    logging.info(
                        f"[AI-ACTION-{execution_id}] Current to_act set: {poker_game.to_act}"
                    )
                    if 0 <= poker_game.current_player_idx < len(poker_game.players):
                        current_p = poker_game.players[poker_game.current_player_idx]
                        logging.info(
                            f"[AI-ACTION-{execution_id}] Current player is: {current_p.name}, in to_act: {current_p.player_id in poker_game.to_act}"
                        )

                    # Now check the current player
                    logging.info(
                        f"[AI-ACTION-{execution_id}] Checking current player after AI action"
                    )
                    ...
                    if poker_game.current_player_idx < len(poker_game.players):
                        ...
                        next_player_domain = next(
                            (p for p in game.players if p.id == next_player.player_id),
                            None,
                        )
                        ...
                        logging.info(
                            f"[AI-ACTION-{execution_id}] Next player determined: {next_player.name} [{next_position_name} - Position {next_player.position}] (index {poker_game.current_player_idx})"
                        )
                        logging.info(
                            f"[AI-ACTION-{execution_id}] Next player status: {next_player.status}, in to_act: {next_player.player_id in poker_game.to_act}"
                        )
                        logging.info(
                            f"[AI-ACTION-{execution_id}] Active players: {len(active_poker_players)}"
                        )
                        logging.info(
                            f"[AI-ACTION-{execution_id}] to_act set content: {poker_game.to_act}"
                        )
                        logging.info(
                            f"[AI-ACTION-{execution_id}] All players position info: {[(p.name, p.position, (p.position - poker_game.button_position) % len(poker_game.players)) for p in poker_game.players]}"
                        )

                        # Verify this player is active and needs to act
                        if (
                            next_player.status == PokerPlayerStatus.ACTIVE
                            ...
                            if not next_player_domain.is_human:
                                logging.info(
                                    f"[AI-ACTION-{execution_id}] Next player is AI ({next_player.name}). AWAITING next action (sequential execution)."
                                )
                                ...
                                await self._request_and_process_ai_action(
                                    game_id, next_player.player_id
                                )
                            else:
                                logging.info(
                                    f"[AI-ACTION-{execution_id}] Next player is Human ({next_player.name}). Sending action request."
                                )
                                await game_notifier.notify_action_request(
                                    game_id, poker_game
                                )
                        ...
                        logging.error(
                            f"AI Action: Invalid current_player_idx after action: {poker_game.current_player_idx}"
                        )

            else:  # process_action failed
                logging.error(
                    f"AI Action Failed: PokerGame.process_action returned False for player {player_id} action {poker_action.name}"
                )

        except Exception as e:
            logging.error(
                f"AI Action Error: Exception during processing for player {player_id}: {str(e)}"
            )
            import traceback
            ...
            if poker_player and poker_player.player_id not in poker_game.to_act:
                logging.error(
                    f"[AI-ACTION-{execution_id}] Fallback aborted: player {player_id} not in to_act. Forcing FOLD."
                )
                poker_action = PokerPlayerAction.FOLD
                ...
                if current_bet == 0:
                    ...
                    if domain_player and domain_player.archetype:
                        ...
                        if any(
                            x in archetype for x in ["LAG", "Maniac", "CallingStation"]
                        ):
                            poker_action = PokerPlayerAction.CALL
            ...
            try:
                ...
                if poker_player and poker_game:
                    logging.warning(
                        f"AI Action Error: Using fallback {fallback_action_name} for player {player_id}"
                    )
                    # Process the fallback action
                    action_success = poker_game.process_action(
                        poker_player, poker_action, None
                    )
                    if not action_success and fallback_action_name != "FOLD":
                        # If the chosen action fails, fall back to FOLD
                        logging.warning(
                            f"AI Action Error: {fallback_action_name} failed, forcing FOLD"
                        )
                        poker_action = PokerPlayerAction.FOLD
                        fallback_action_name = "FOLD"
                    ...
                    await game_notifier.notify_player_action(
                        ...
                        total_hand_bet=(
                            fs_post_hand
                            if fallback_action_name.upper() in ["ALL_IN", "ALL-IN"]
                            else None
                        ),
                    )
                    await game_notifier.notify_game_update(game_id, poker_game)
                    ...
                    logging.error(
                        f"AI Action Error: Cannot process action because poker_player or poker_game is invalid."
                    )
            except Exception as action_error:
                logging.error(
                    f"AI Action Error: Failed to process {fallback_action_name} action after error: {action_error}"
                )

        logging.info(
            f"[AI-ACTION-END-{execution_id}] Finished AI Action for Player {player_id} - Duration: {time.time() - start_time:.2f}s"
        )
    ...
    def rebuy_player(self, game_id: str, player_id: str, amount: int) -> Player:
        ...
        if new_total > game.cash_game_info.max_buy_in:
            raise ValueError(
                f"Rebuy would exceed maximum buy-in of {game.cash_game_info.max_buy_in}"
            )
    ...
    def top_up_player(self, game_id: str, player_id: str) -> Tuple[Player, int]:
        ...
        if current_chips >= max_buy_in:
            raise ValueError(
                f"Player already has maximum chips ({current_chips} >= {max_buy_in})"
            )
