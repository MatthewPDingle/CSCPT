...
class HandHistoryRecorder:
    ...
    def start_hand(
        ...
        for player in players:
            ...
            snapshot = PlayerHandSnapshot(
                ...
                hole_cards=(
                    [str(c) for c in player.hand.cards]
                    if hasattr(player.hand, "cards")
                    else []
                ),
            )
        ...
        if betting_round.name == "PREFLOP":
            for p in self.current_hand.players:
                if p.player_id == player_id:
                    if domain_action in [
                        DomainPlayerAction.CALL,
                        DomainPlayerAction.BET,
                        DomainPlayerAction.RAISE,
    ...
    def record_pot_results(self, pots: List[Pot], hand_evaluations: Dict) -> None:
        ...
        for i, pot in enumerate(pots):
            pot_result = PotResult(
                ...
                amount=pot.amount,
                eligible_players=(
                    list(pot.eligible_players)
                    if hasattr(pot, "eligible_players")
                    else []
                ),
            )
            ...
            if pot_result.winners and f"{pot_key}_hand" in hand_evaluations:
                ...
                pot_result.winning_hand_type = (
                    winning_hand.name
                    if hasattr(winning_hand, "name")
                    else str(winning_hand)
                )
        ...
        if MEMORY_SYSTEM_AVAILABLE:
            try:
                ...
                if "MemoryIntegration" in globals():
                    ...
                    print(
                        "MemoryIntegration not available, skipping hand history processing"
                    )
            except Exception as e:
                # Don't let memory errors disrupt the game
    ...
    def _update_metrics(
        self, action: ActionDetail, betting_round: BettingRound
    ) -> None:
        """
        ...
        if betting_round.name == "FLOP":
            ...
            if last_aggressor == action.player_id:
                if action.action_type in [
                    ...
                    if all(
                        a.action_type == DomainPlayerAction.FOLD
                        for a in flop_actions
                        if a.player_id != action.player_id
                        and a.position_in_action_sequence
                        > action.position_in_action_sequence
                    ):
        ...
        if action.all_in:
            all_in_count = sum(
                ...
                for a in [
                    a
                    for round_actions in self.current_hand.betting_rounds.values()
