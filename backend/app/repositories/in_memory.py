...
class HandHistoryRepository(InMemoryRepository[HandHistory]):
    ...
    def get_player_stats(
        self, player_id: str, game_id: Optional[str] = None
    ) -> PlayerStats:
        """
        ...
        for hand in hands:
            ...
            if preflop_actions and preflop_actions[-1].player_id == player_id:
                ...
                if flop_actions:
                    first_to_act = next(
                        (a for a in flop_actions if a.player_id == player_id), None
                    )
                    if first_to_act and first_to_act.action_type in [
                        PlayerAction.BET,
                        PlayerAction.RAISE,
                    ]:
                        cbet_attempts += 1
                        if all(
                            a.action_type == PlayerAction.FOLD
                            for a in flop_actions
                            if a.player_id != player_id
                            and a.position_in_action_sequence
                            > first_to_act.position_in_action_sequence
                        ):
                            cbet_successes += 1
