...
class TestGameWebSocketApi:
    ...
    async def test_process_action_message(
        ...
        message = {"type": "action", "data": {"action": "CALL", "amount": 10}}
        ...
        await process_action_message(
            mock_websocket, game_id, message, player_id, mock_service
        )
    ...
    @pytest.mark.asyncio
    async def test_process_action_message_invalid_action(
        self, mock_service, mock_game, mock_poker_game, mock_connection_manager
    ):
        ...
        message = {
            "type": "action",
            "data": {"action": "RAISE", "amount": 20},  # Not in valid actions
        }
    ...
    @pytest.mark.asyncio
    async def test_process_action_message_not_players_turn(
        self, mock_service, mock_game, mock_poker_game, mock_connection_manager
    ):
        ...
        await process_action_message(
            mock_websocket, game_id, message, player1_id, mock_service
        )
    ...
    async def test_process_chat_message(
        ...
        # Create WebSocket message
        message = {"type": "chat", "data": {"text": "Hello, world!", "target": "table"}}
