...
def mock_memory_integration():
    ...
    with patch("ai.memory_integration.MemoryIntegration") as mock:
        ...
        mock.get_agent_decision = AsyncMock(
            return_value={"action": "fold", "reasoning": {}}
        )
...
async def test_ai_turn_triggering(reset_game_service, mock_memory_integration):
    ...
    game = service.create_cash_game(
        name="Test Game", min_buy_in=40, max_buy_in=100, small_blind=1, big_blind=2
    )

    # Add a human player
    human_player, _ = service.add_player(
        game_id=game.id, name="Human Player", is_human=True, position=0
    )

    # Add an AI player
    ai_player, _ = service.add_player(
        game_id=game.id, name="AI Player", is_human=False, archetype="TAG", position=1
    )
    ...
    with patch("app.core.websocket.GameStateNotifier") as mock_notifier:
        ...
        from_domain_current_player = next(
            (p for p in game.players if p.id == current_player_id), None
        )
...
async def test_consecutive_ai_turns(reset_game_service, mock_memory_integration):
    ...
    ai_player1, _ = service.add_player(
        game_id=game.id, name="AI Player 1", is_human=False, archetype="TAG", position=0
    )

    ai_player2, _ = service.add_player(
        game_id=game.id, name="AI Player 2", is_human=False, archetype="LAG", position=1
    )
    ...
    with patch("app.core.websocket.GameStateNotifier") as mock_notifier:
        ...
        mock_memory_integration.get_agent_decision.return_value = {
            "action": "check",
            "reasoning": {},
        }
...
async def test_error_handling_in_ai_action(reset_game_service, mock_memory_integration):
    ...
    # Add an AI player
    ai_player, _ = service.add_player(
        game_id=game.id, name="AI Player", is_human=False, archetype="TAG", position=0
    )

    # Add another player to keep the game going
    human_player, _ = service.add_player(
        game_id=game.id, name="Human Player", is_human=True, position=1
    )
    ...
    with patch("app.core.websocket.GameStateNotifier") as mock_notifier:
        ...
        game_notifier.notify_player_action.assert_called_once_with(
            game.id, current_player_id, "FOLD", None
        )
        game_notifier.notify_game_update.assert_called_once()

        # Verify the player status is now folded
        player_in_game = next(
            (p for p in poker_game.players if p.player_id == current_player_id), None
        )
