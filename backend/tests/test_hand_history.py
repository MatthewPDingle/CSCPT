...
class TestHandHistory(unittest.TestCase):
    ...
    def test_complete_hand(self):
        ...
        # Verify player2 won
        player2_snapshot = next(p for p in hand_history.players if p.player_id == "player2")
    ...
    def test_early_hand_ending(self):
        ...
        player3_snapshot = next((p for p in hand_history.players if p.player_id == "player3"), None)
    ...
    def test_all_in_confrontation(self):
        ...
        preflop_actions = hand_history.betting_rounds["PREFLOP"]
        all_in_action = next((a for a in preflop_actions if a.player_id == "player3"), None)
        ...
        expected_pot = (3 * 5) + 10 + 20 + 40 + 100 + 80  # ante + SB + BB + initial raise + all-in + call
        total_pot = sum(pot.amount for pot in hand_history.pot_results)
        self.assertEqual(total_pot, expected_pot)
        
        # Check player3's eligibility is limited to the main pot
        pots_by_size = sorted(hand_history.pot_results, key=lambda p: p.amount, reverse=True)
        if len(pots_by_size) >= 2:
            ...
            if "player3" in main_pot.eligible_players:
                ...
                # Check by pot names
                main_pot = next((p for p in hand_history.pot_results if "Main" in p.pot_name), None)
                if main_pot:
    ...
    def test_game_with_multiple_hands(self):
        ...
        self.assertNotEqual(
            hands_by_number[0].dealer_position,
            hands_by_number[1].dealer_position
        )
