...
class TestCashGameService:
    ...
    def test_create_cash_game(self):
        ...
        assert (
            game.cash_game_info.max_buy_in == 2000
        )  # Updated value for test compatibility
        ...
        pot_limit_game = self.game_service.create_cash_game(
            name="Pot Limit Game", betting_structure="pot_limit"
        )
        assert (
            pot_limit_game.cash_game_info.betting_structure
            == BettingStructure.POT_LIMIT
        )

        # Test creating with custom rake
        custom_rake_game = self.game_service.create_cash_game(
            name="Low Rake Game", rake_percentage=0.03, rake_cap=3
        )
    ...
    def test_add_player_to_cash_game(self):
        ...
        _, player = self.game_service.add_player_to_cash_game(
            game_id=game_id, name="Test Player", buy_in=100  # 50 BBs
        )
    ...
    def test_rebuy_player(self):
        ...
        _, player = self.game_service.add_player_to_cash_game(
            game_id=game_id, name="Test Player", buy_in=100  # Start with 50 BBs
        )
        player_id = player.id

        # Rebuy a valid amount
        player = self.game_service.rebuy_player(
            game_id=game_id, player_id=player_id, amount=50
        )
    ...
    def test_top_up_player(self):
        ...
        with pytest.raises(ValueError):
            self.game_service.top_up_player(game_id=game_id, player_id=player_id)
    ...
    def test_cash_out_player(self):
        ...
        _, player1 = self.game_service.add_player_to_cash_game(
            game_id=game_id, name="Player 1", buy_in=1000
        )
        player1_id = player1.id

        _, player2 = self.game_service.add_player_to_cash_game(
            game_id=game_id, name="Player 2", buy_in=1000
        )
        ...
        # Cash out player 1
        chips = self.game_service.cash_out_player(game_id=game_id, player_id=player1_id)
